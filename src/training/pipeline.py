...
class TrainingPipeline(BasePipeline[BuildResult, None]):
    def __init__(self, training_cfg: TrainingStageConfig):
    ...
    def build(self) -> BuildResult:
        ...
        """
        PathManager.ensure_dir(self.training_cfg.trainig_dir.output_dir)
        model_class, _ = get_model_class_and_short(self.training_cfg.model.name)
        writers = IOFactory.create_writers()
        readers = IOFactory.create_readers()
        training = TrainingFactory.create(model_class, self.training_cfg)
        return BuildResult(
            model_class=model_class, writers=writers, readers=readers, training=training
        )

    def load_data(self, readers: Readers) -> SplitData:
        """
        ...
        return data_loader.load_splitted_data(
            processed_dir=self.training_cfg.data_dir.processed_dir
        )
    ...
    def run(self) -> None:
        ...
        setup_mlflow()
        builder = self.build()
        split_data = self.load_data(builder.readers)
        data_saver = DataSaver(builder.writers)
        run_saver = RunSaver(
            trainig_dir=self.training_cfg.trainig_dir, data_saver=data_saver
        )
